...
class Node(object): #Initialize the Node class

	def __init__(self, up, down, left, right, x, y, value=None): """Types:
									up: node
									down: node
									left: node
									right: node
...
class Header(Node): 
	...
	def __init__(self, up, down, left, right, primary): #Initialize Header of the Linked List Matrix
		super(Header, self).__init__(up, down, left, right, None, None)

		self.is_primary = primary

	def tally_values(self): #Add all values in linked list column together
...
class Matrix:
	...
	def __init__(self, values): """Inilialize Matrix
					type values: int array
					"""
		self.removed_nodes = []
	...
	def add_row(self, values): """add a row to the Matrix
					type values: int array
					"""
	...
	def get_number_of_headers(self): #Return total number of headers in the matrix
	...
	def tally_all_values(self): #Add all values in Matrix together
		current_header = self.first_header
	...
	def remove_row(self, index): #Remove specified row from matrix
	...
	def remove_row_overlap(self, index): #Remove all nodes found on the removed nodes list from their rows
	...
	def restore_row(self, first_node): #Add a number to the list from the removed nodes list after aligning row
	...
	def restore_row_overlap(self, index): #Add nodes from the removed node list to the current index via row
		for i in range(0, len(self.removed_nodes[index])):
			current_node = self.removed_nodes[index][i]
			if current_node is not None:
				current_node.up.down = current_node
	...
	def remove_column(self, index): #Remove a column from the matrix
	...
	def remove_column_overlap(self, index): #Remove all nodes on the removed nodes list from their columns
	...
	def restore_column(self, first_node): #Add all removed nodes in first_node's column
		current_node = first_node
	...
	def restore_column_overlap(self, index): #Add all nodes in the index column to the column
		for i in range(0, len(self.removed_nodes)):
			current_node = self.removed_nodes[i][index]
			if current_node is not None:
	...
	def get_array_representation(self): #Return an array of all the values in the matrix
